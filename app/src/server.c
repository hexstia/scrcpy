...
static process_t execute_server(const char *serial,
    ...
    const char *const cmd[] = {
       ...
       "CLASSPATH=/system/framework/scrcpy-server.jar",
        "app_process",
        "/", // unused
...
*/
    server->tunnel_forward = SDL_FALSE;
    ...
    if (!server->tunnel_forward) {
        ...
        if (server->server_socket == INVALID_SOCKET) {
            printf("Could not listen on port %d",local_port);
    ...
    printf("android programe not use!!!!\n");
        if (!server->tunnel_forward) {
...
socket_t server_connect_to(struct server *server) {
    printf("server_connect_to() comming \n");
    if (!server->tunnel_forward) {
        printf("if !server->tunnel_forward");
        server->device_socket = net_accept(server->server_socket);
        printf("accept create link  and client");
    } else {
        Uint32 attempts = 100;
        Uint32 delay = 100; // ms
